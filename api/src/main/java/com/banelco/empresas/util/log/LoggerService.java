...
public class LoggerService {
	...
	@Around("execution(* com.banelco.empresas.service..*(..)) || "
            + "execution(* com.banelco.empresas.repository..*(..))")
    public Object logTimeMethod(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (interfaces.length > 1) {
            for (Class iface : interfaces) {
                if (iface.getName().startsWith("com.banelco.empresas.repository")) {
                    logMessage.append(iface.getSimpleName());
                }
            }
        } else {
